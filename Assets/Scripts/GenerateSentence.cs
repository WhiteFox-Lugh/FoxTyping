...
public class GenerateSentence {
	...
	private List<(string jp, string h)> qJP1 = new List<(string jp, string h)>() {
		...
		("やられたらやり返す、", "やられたらやりかえす、"), ("賽は", "さいは"),
		("我輩の辞書に", "わがはいのじしょに"), ("我輩は", "わがはいは"),
		("なるほど、それは", "なるほど、それは"), ("お値段異常", "おねだんいじょう"),
		("増税前に", "ぞうぜいまえに"), ("犬も歩けば", "いぬもあるけば"), ("優勝商品は", "ゆうしょうしょうひんは"),
		("世にも奇妙な", "よにもきみょうな"), ("精神的に向上心のないものは", "せいしんてきにこうじょうしんのないものは"),
		("不正をしないと勝てない", "ふせいをしないとかてない"), ("イージーモードが許されるのは", "いーじーもーどがゆるされるのは"),
		...
		("選ばれたのは", "えらばれたのは"), ("匠の技が光る", "たくみのわざがひかる"),
		("ユーザインタフェースが最悪な", "ゆーざいんたふぇーすがさいあくな"), ("唐揚げにレモンをかける", "からあげにれもんをかける"),
		("賞味期限間近の", "しょうみきげんまぢかの"), ("出でよ、", "いでよ、"), ("ナイス", "ないす"),
	...
	private List<(string jp, string h)> qJP2 = new List<(string jp, string h)>() {
		...
		("明智光秀", "あけちみつひで"), ("お前だったのか", "おまえだったのか"), ("廃藩置県", "はいはんちけん"),
		("鹿苑寺金閣", "ろくおんじきんかく"), ("慈照寺銀閣", "じしょうじぎんかく"), ("藤原道長", "ふじわらのみちなが"),
		...
		("力尽きました", "ちからつきました"), ("遊ぶ金が欲しかった", "あそぶかねがほしかった"), ("あおり運転", "あおりうんてん"),
		("ネコを吸う", "ねこをすう"), ("校長先生", "こうちょうせんせい"), ("留年", "りゅうねん"),
		("逮捕されました", "たいほされました"), ("爆発しました", "ばくはつしました"), ("進捗どうですか", "しんちょくどうですか"),
		("お楽しみください", "おたのしみください"), ("斎藤さん", "さいとうさん"), ("鈴木さん", "すずきさん"),
		("アセチレン", "あせちれん"), ("ゴミ", "ごみ"), ("ゴリラ", "ごりら"), ("パイソン", "ぱいそん"),
	...
	public (string jp, string hi, List<string> hiSep, List<List<string>> ty) Generate(int g){
		...
		while(!isOK){
			try {
				...
				int r2 = UnityEngine.Random.Range(0, qJP2.Count);
				jpStr = ((gameModeEasy == g) ? "" : qJP1[r1].jp) + qJP2[r2].jp;
